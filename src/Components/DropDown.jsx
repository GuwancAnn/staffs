...
import "./style.css";
const Wdrop = ({ departments, setstaffs, staffs }) => {
  const fData = async (id) => {
    console.log(id);

    const { data, error } = await supabase
      .from("staffs")
      .select(`*,professions(*),departments(*), departments_id`)
      .filter("departments_id", "in", `(${id})`);
    console.log(data);
    console.log(error);
    setstaffs(data);
  };
  ...
  const items = departments.map((department) => {
    return {
      label: (
        <button className="drop-btn" onClick={() => fData(department.id)}>
          {department.dep_name}
        </button>
      ),
    };
